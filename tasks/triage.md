...
- scripts/mods/mod_hardened_qol_fork/perk_groups/** → DELETE (pg_hd__, pg_special_hd__)
...
- scripts/skills/special/ KEEP (7): hd_bag_item_silhouettes.nut, hd_frenzy_eyes_manager.nut, hd_dummy_morale_state.nut + _breaking/_confident/_fleeing/_wavering.nut (QoL infra referenced by KEEP hooks). DELETE (5): hd_bag_item_manager (balance bag-slot normalization, needs removed BagSlots property), hd_direct_damage_limiter, hd_non_combatant_effect, hd_worthless_effect, hd_unworthy_effect.
- scripts/scenarios/tactical/scenario_hd_* (12) → DELETE. scripts/entity/world/hd_waypoint.nut → **KEEP** (REVERSED 2026-08-04: QoL `DisplayWaypoint` marker; purge caused null spawn → 209x init + null-cascade). scripts/entity/tactical/enemies/hd_trickster_hollenhund.nut → DELETE. scripts/ai/tactical/behaviors/hd_* (3) → DELETE. scripts/ai/tactical/agents/hd_* (5) → DELETE.
