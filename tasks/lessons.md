...
- **Local tag namespace is flat — fork `1.22.4` replaces the upstream-fetched `1.22.4`.** After re-tagging, `git fetch upstream --tags` will re-point `1.22.4` at the upstream commit and silently orphan the release anchor. Fetch upstream with `--no-tags` (documented in `.agents/build-release-process/SKILL.md`).
