...
## 2026-09-07 — discover_location crash (purge-over-delete, contract side)

- **A KEEP'd hook can still carry the deleted rework's mechanism inside it.** `discover_location_contract.nut` was triaged KEEP for its anti-cheese cache/serialize directions, but its `q.setup` override was the _target-selection mock_ of the deleted balance rework (`discover_location_action.nut`, purged with `HD_ScoutableFactions`/`HD_ScoreOverwrite`/`HD_MaximumDistance`). It `::new()`'d a fresh `discover_location_action` and read `.m.HD_ScoutableFactions` → `the index 'HD_ScoutableFactions' does not exist` every time the contract's setup ran (4x, plus 2 unrelated `getTile` index errors in a vanilla contract file). Fix = delete the stale `q.setup` override entirely (returns to vanilla Undead/Zombie scouting), NOT restore the member — restoring re-imports the all-faction/renown-chance rework = balance churn. When a kept file crosses a deleted rework, check whether the kept surface is only the header-documented QoL; if so, strip the leftover mechanism, don't resurrect the seed.
