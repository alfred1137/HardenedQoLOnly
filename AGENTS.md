...
- Upstream reference: `https://github.com/Darxo/Hardened`; this fork tracks its `develop` (base upstream release `1.22.0`, incorporates upstream `1.22.1` null-guard fixes and `1.22.5` QoL).
...
- **Dragonslayer easter egg (PROTECTED, do not purge):** gated by MSU setting `DragonslayerEasterEgg` (default OFF; page label "Easter Egg"). Consists of `scripts/items/weapons/hd_dragonslayer.nut`, 4 actives (`hd_guillotine_strike`, `hd_steel_crescent`, `hd_decimate`, `hd_demolish_shield`), marketplace stock hook (`mod_hardened_qol_fork/hooks/entity/world/settlements/buildings/marketplace_building.nut`), gfx/brushes. Files carry `Feat:` headers but are INTENTIONAL content port, not balance churn — treat as KEEP. Original creator: Koltira, a Scaly Grumpiness (port specs: `tasks/dragonslayer_design.md`, `tasks/dragonslayer_todo.md`, source `doc/mod_guts_dragonslayer_v1.0.4/`).
